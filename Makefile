...
top_builddir = /home/phdyex/my-repos/cygwin/bash
srcdir = .
...
BASH_IMPLIB = libcygbash.dll.a
DLL_LDFLAGS = -shared -Wl,--export-all-symbols -Wl,--enable-auto-import \
	-Wl,--out-implib,$(BASH_IMPLIB)
...
CPPFLAGS_FOR_BUILD = -I/home/phdyex/include
LOCAL_CFLAGS = -DRECYCLES_PIDS ${DEBUG} ${MALLOC_DEBUG}
DEFS = -DHAVE_CONFIG_H
LOCAL_DEFS = -DSHELL
...
OBJECTS	 = shell.o eval.o y.tab.o general.o make_cmd.o print_cmd.o $(GLOBO) \
	   ...
	   pcomplete.o pcomplib.o syntax.o xmalloc.o $(SIGNAMES_O) command_hooks.o

# Objects for DLL build (includes shell_dll.o with main renamed to bash_main)
DLL_OBJECTS = shell_dll.o eval.o y.tab.o general.o make_cmd.o print_cmd.o $(GLOBO) \
	   dispose_cmd.o execute_cmd.o variables.o copy_cmd.o error.o \
	   expr.o flags.o $(JOBS_O) subst.o hashcmd.o hashlib.o mailcheck.o \
	   trap.o input.o unwind_prot.o pathexp.o sig.o test.o version.o \
	   alias.o array.o arrayfunc.o assoc.o braces.o bracecomp.o bashhist.o \
...
# Monolithic build (original, for non-Cygwin platforms)
$(Program)-monolithic:  .build $(OBJECTS) $(BUILTINS_DEP) $(LIBDEP)
	$(RM) $@
	$(PURIFY) $(CC) $(BUILTINS_LDFLAGS) $(LIBRARY_LDFLAGS) $(LDFLAGS) -o $(Program) $(OBJECTS) $(LIBS)
	ls -l $(Program)
	-$(SIZE) $(Program)

# shell_dll.o - shell.c compiled with main() renamed to bash_main() for DLL
shell_dll.o: shell.c
	$(RM) $@
	$(CC) $(CCFLAGS) -Dmain=bash_main -c -o $@ $(srcdir)/shell.c

# shell_stub.o - tiny main() that calls bash_main() in the DLL
shell_stub.o: shell_stub.c
	$(RM) $@
	$(CC) $(CCFLAGS) -c -o $@ $(srcdir)/shell_stub.c

# Cygwin DLL build - creates cygbash-X.Y.dll with exported symbols
$(BASH_DLL): .build $(DLL_OBJECTS) $(BUILTINS_DEP) $(LIBDEP)
	$(RM) $@ $(BASH_IMPLIB)
	$(CC) $(DLL_LDFLAGS) $(BUILTINS_LDFLAGS) $(LIBRARY_LDFLAGS) $(LDFLAGS) \
		-o $@ $(DLL_OBJECTS) $(LIBS)
...
clean:	basic-clean
	...
	-( cd $(PO_DIR) ; $(MAKE) $(MFLAGS) DESTDIR=$(DESTDIR) $@ )
	-( cd $(LOADABLES_DIR) && $(MAKE) $(MFLAGS) DESTDIR=$(DESTDIR) $@ )
	-( test -d bash-server && cd bash-server && test -f Makefile && $(MAKE) $(MFLAGS) $@ )
	-( test -d bashclient && cd bashclient && test -f Makefile && $(MAKE) $(MFLAGS) $@ )
	$(RM) $(CREATED_SUPPORT)
...
# bash-server targets (built when --enable-bash-server is specified)
.PHONY: bash-server bashclient

bash-server: .made
	@if test -d bash-server && test -f bash-server/Makefile; then \
		echo "Building bash-server..."; \
		( cd bash-server && $(MAKE) $(MFLAGS) all ); \
	else \
		echo "bash-server not configured (use --enable-bash-server)"; \
	fi

bashclient: .made
	@if test -d bashclient && test -f bashclient/Makefile; then \
		echo "Building bashclient..."; \
		( cd bashclient && $(MAKE) $(MFLAGS) all ); \
	else \
...
execute_cmd.o: command_hooks.h
command_hooks.o: config.h bashansi.h ${BASHINCDIR}/ansi_stdlib.h ${BASHINCDIR}/stdc.h
command_hooks.o: command_hooks.h
