...
public class ControlChannel
{
    ...
    public async Task<Msg> AuthAsync(string token, CancellationToken ct = default)
    {
        await _send(NdjsonProtocol.MakeMsg(Ch.Control, "auth",
            new() { ["token"] = token }));
        var resp = await _recv(Ch.Control, ct);
    ...
    {
        await _send(NdjsonProtocol.MakeMsg(Ch.Control, "ping"));
        var resp = await _recv(Ch.Control, ct);
    ...
    public async Task<Msg> ConfigureAsync(object opts, CancellationToken ct = default)
    {
        ...
        await _send(NdjsonProtocol.MakeMsg(Ch.Control, "configure", extra));
        return await _recv(Ch.Control, ct);
    }

    public async Task DisconnectAsync(CancellationToken ct = default)
    {
        await _send(NdjsonProtocol.MakeMsg(Ch.Control, "disconnect"));
        try { await _recv(Ch.Control, ct); } catch { }
    }
}
